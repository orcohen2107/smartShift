...
export default function Dropdown({
  ...
  const selectedItem = value != null ? items.find((i) => i.value === value) : null;
  const displayLabel =
  ...
  return (
    <Menu as="div" className={`relative block w-full ${className ?? ""}`}>
      ...
      >
        ...
        <ChevronDownIcon aria-hidden className="h-4 w-4 shrink-0 text-zinc-400" />
      </MenuButton>
        ...
        <div className="py-1">
          {items.map((item) => {
            ...
            const itemClass =
              ...
              (isSelected ? " bg-emerald-50 text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-200" : "");

            const content = <span className="min-w-0 truncate">{item.label}</span>;
